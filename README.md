...
- **时间范围只按登记时间 `createdAt` 判定**：缺损项的修复时间 `repairedAt`、批次的完成时间
  `completedAt` 不参与。例如六月登记、九月修好的缺损，不会出现在九月区间的查询里。
...
```json
{
  ...
  "tookMs": 3,
  "generation": 2,
  "page": { "sessionId": "s_…", "pageSize": 20, "cursor": null, "nextCursor": "ey…（不透明游标）", "hasMore": true, "index": 1 }
}
```

### 分页（会话 O(1) 内存，翻页期间增删改不重不漏）

首次请求不带 `sessionId`，服务端为这次查询建立一个**时间点会话**（默认保留 10 分钟，
翻页自动续期），返回 `page.sessionId` 与不透明的 `page.nextCursor`（base64url，记录上一页
最后一条的排序键，请原样回传、不要自行构造）。后续翻页带上二者：

```bash
curl '/search?q=碑刻&pageSize=20'
curl '/search?q=碑刻&pageSize=20&sessionId=s_xxx&cursor=eyJzIjoxMDAsLi4ufQ'
```

实现上会话**只保存查询参数、seq 与游标（约百余字节），不保存结果列表**，因此常驻内存
不随命中条数线性增长（实测十个万级结果查询的会话总计约 1KB）。每次翻页在「会话建立时
那个 seq 的一致性视图」上重跑查询并用 keyset 游标定位窗口；该视图由一段**短窗口多版本
历史**支撑，历史只保存会话存活期间被改动的少量记录的旧版本，会话全部结束即清空，
占用只与这段时间的写入量有关、与任何结果条数无关。

因此翻页过程中即使有新增、修改、删除：

- 已在翻的结果**不会重复、不会漏项、顺序不乱**，且始终展示**查询时刻**的字段值；
- 会话期间某条被删除或改成不匹配：本会话仍按查询时刻照常返回它（时间点快照语义）；
...
```bash
npm test          # node --test tests/，共 59 个用例
```

覆盖：归一化与模糊匹配、组合查询、命中原因、评分与稳定排序、时间范围只按登记时间、
会话分页不重不漏且会话占用 O(1)、写入立即可查、WAL 持久化/压实/残缺自愈、旧库迁移、
重建中断续跑、重建并发隔离、幂等重建、健康检查与旧接口逐字段兼容、HTTP 端到端，
以及 10 万条数据下的查询/分页性能与会话内存（断言查询 < 1s）。
